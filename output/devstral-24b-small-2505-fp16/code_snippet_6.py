...
def check_dataset_balance(output_path=OUTPUT_PATH):
    ...
    missing_dict (dict): A dictionary where keys are PLU numbers and values are the number of
                         images missing to reach the maximum count or 300, whichever is smaller.
    is_balanced (bool): True if every category has at least 300 images, False otherwise.
    """
    ...
    # Set target count to 300 or max_count, whichever is smaller
    max_count = max(category_counts.values())
    target_count = min(max_count, 300)
...
def augment_cropped_images(missing_dict, output_path='out_data/', num_folders=None, num_files_per_folder=None):
    ...
    (rotations, horizontal flip, vertical flip, and a combined flip + rotation) only if the folder
    has fewer than 300 PNG images.
    Args:
        missing_dict (dict): Dictionary with PLU codes as keys and number of missing images as values.
        output_path (str): Path to the directory containing cropped images.
        num_folders (int, optional): Limit processing to this number of folders. Defaults to None.
        num_files_per_folder (int, optional): Limit augmentations per folder to this number of files. Defaults to None.
    """
    ...
    for folder in folders:
        ...
        # Check if folder needs augmentation
        plu_code = folder  # Assuming the folder name is the PLU code
        if missing_dict.get(plu_code, 0) <= 0:
            print(f"Skipping folder '{folder}' because it doesn't need augmentation.")
            continue
...
if __name__ == '__main__':
    ...
    if not balanced:
        augment_cropped_images(missing_dict, output_path=OUTPUT_PATH,
