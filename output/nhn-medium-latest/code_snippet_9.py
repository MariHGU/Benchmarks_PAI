...
def check_dataset_balance(output_path=OUTPUT_PATH):
    """Checks whether each category (identified by the filename prefix, assumed to be the PLU number) in
    the output folder has the same number of images as the category with the highest count.

    Returns:
        missing_dict (dict): A dictionary where keys are PLU numbers and values are the number of
                             images missing to reach the maximum count.
...
def augment_cropped_images(output_path='out_data/', num_folders=None, num_files_per_folder=None):
    ...
    (rotations, horizontal flip, vertical flip, and a combined flip + rotation) only if the folder has less than 300 PNG images.
    """
    ...
    for folder in folders:
        ...
        if len(total_png_files) >= 300:  # Changed from 200 to 300 as per your requirement
        ...
        # Continue augmenting until the folder has around 300 images
        while len(total_png_files) < 300:  # Changed from 200 to 300 as per your requirement
            for cropped_file in cropped_files:
                ...
                for angle in [90, 180, 270]:
                    ...
                    total_png_files.append(f"{base_name}_cropped_rot{angle}.png")
                ...
                total_png_files.append(f"{base_name}_cropped_flipH.png")
                ...
                total_png_files.append(f"{base_name}_cropped_flipV.png")
                ...
                total_png_files.append(f"{base_name}_cropped_flipH_rot90.png")
